...
const BooleRuleSolver = () => {
  const [demoInProgress, setDemoInProgress] = useState(false);
  const [functionInput, setFunctionInput] = useState('1 / (1 + x)');
  ...
  const calculateBoole = (fExpr, aVal, bVal, nVal) => {
    setError('');
    ...
    if (n % 4 !== 0) {
      setError('Boole\'s Rule requires subintervals (n) to be a multiple of 4.');
      return;
    }
    if (a >= b) {
    ...
    try {
      ...
      setError('Invalid function input. Please enter a valid mathematical expression like 1 / (1 + x).');
      return;
    }

    const h = (b - a) / n;
    const log = [];
    log.push(`Interval [a, b] = [${a}, ${b}], n = ${n} (Multiple of 4)`);
    log.push(`Grid Step Size h = (${b} - ${a}) / ${n} = ${h.toFixed(6)}`);

    const points = [];
    const getBooleWeight = (idx, totalN) => {
      if (idx === 0 || idx === totalN) return 7;
      const mod = idx % 4;
      if (mod === 1 || mod === 3) return 32;
      if (mod === 2) return 12;
      return 14; // boundary between blocks of 4
    };
    ...
    for (let i = 0; i <= n; i++) {
      const x = a + i * h;
      const y = f(x);
      const weight = getBooleWeight(i, n);
    ...
    const integralValue = ((2 * h) / 45) * totalWeightedSum;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    calculateBoole(functionInput, lowerLimit, upperLimit, subintervals);
  };

  const handleDemo = () => {
    setDemoInProgress(true);
    setFunctionInput('1 / (1 + x)');
    setLowerLimit(0);
    setUpperLimit(4);
    setSubintervals(4);
    calculateBoole('1 / (1 + x)', 0, 4, 4);
  ...
  return (
    <div className="w-full mx-auto p-6 bg-white dark:bg-neutral-800 dark:text-white rounded-2xl shadow-xl border border-gray-100 dark:border-neutral-700">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Boole&apos;s Rule Solver</h1>
        <TButton
          tooltipText="Demo"
          onClick={handleDemo}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label htmlFor="subintervals" className="block text-sm font-semibold mb-1">Subintervals <InlineMath math="n" /> (Multiple of 4):</label>
            <input
              type="number"
              id="subintervals"
              min="4"
      ...
      {plotPoints.length > 0 && (
        <div className="mt-6 p-6 dark:bg-neutral-700 rounded-2xl border border-gray-200 dark:border-neutral-600">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">Boole&apos;s Rule Curve Plot:</h2>
            <ExportToPNG
              elementId="graphCanvas"
              fileName="boole_plot.png"
              tooltipText="Export Plot to PNG"
              color="blue"
            />
          </div>
          <div id="graphCanvas">
            <Plot points={plotPoints} title={`Boole's Rule Fit (n = ${subintervals})`} />
          </div>
        </div>
      )}
      ...
      {stepsData.length > 0 && (
        <div className="my-6 overflow-x-auto">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">Boole&apos;s Table of Grid Values:</h2>
            <ExportToPNG
              elementId="Table"
              fileName="boole_table.png"
          ...
          <table id="Table" className="w-full table-auto border-collapse border dark:border-neutral-600 text-left text-sm">
            <thead>
              <tr className="bg-gray-100 dark:bg-neutral-700">
                ...
                <th className="border p-3">Boole Weight</th>
            ...
            <tbody>
              {stepsData.map((row) => (
                <tr key={row.index} className="hover:bg-gray-50 dark:hover:bg-neutral-700/50">
                  ...
                  <td className="border p-3 font-bold text-indigo-600 dark:text-indigo-400">{row.weight}</td>
        ...
        <div className="mt-6 p-6 dark:bg-neutral-700 rounded-2xl border border-gray-200 dark:border-neutral-600 space-y-4">
          ...
          <div id="steps" className="space-y-4 font-mono">
            ...
            <div className="p-4 bg-indigo-50 dark:bg-neutral-900 rounded-xl border border-indigo-300 dark:border-indigo-700">
              <h3 className="text-lg font-bold text-indigo-800 dark:text-indigo-300 mb-2">Final Integration Formula Substitution</h3>
              <BlockMath math={`I = \\frac{2h}{45} \\sum_{i=0}^{n} w_i \\cdot y_i`} />
              <BlockMath math={`I = \\frac{2(${result.h.toFixed(6)})}{45} \\left[ ${result.totalWeightedSum.toFixed(6)} \\right]`} />
...
export default BooleRuleSolver;
